...
beforeAll(async () => {
  ...
  organizationId = (
    await createTestOrganization({ suffix: `pred-${suffix}`, ownerId })
  ).id;
  pageId = (await createTestPage({ organizationId, suffix: `pred-${suffix}` }))
    .id;
  blockId = (await createTestBlock({ pageId, type: 'content' })).id;

  strangerOrganizationId = (
    await createTestOrganization({
      suffix: `pred-stranger-${suffix}`,
      ownerId: strangerId,
    })
  ).id;
  strangerPageId = (
  ...
  strangerBlockId = (
    await createTestBlock({ pageId: strangerPageId, type: 'content' })
  ).id;
});
...
describe('userIsMemberOfOrg', () => {
    ...
    const rows = await db
      ...
      .where(
        and(
          ...
          userIsMemberOfOrg(organization.id, ownerId)
        )
      );
  ...
  it('does not match for a non-member', async () => {
    const rows = await db
      ...
      .where(
        and(
          eq(organization.id, organizationId),
          userIsMemberOfOrg(organization.id, strangerId)
        )
      );
  ...
  it('can require a role', async () => {
    const asAdmin = await db
      ...
      .where(
        and(
          eq(organization.id, organizationId),
          userIsMemberOfOrg(organization.id, ownerId, 'admin')
        )
      );
  ...
  it('does not match the stranger owning a different organization', async () => {
    const rows = await db
      ...
      .where(
        and(
          eq(organization.id, strangerOrganizationId),
          userIsMemberOfOrg(organization.id, ownerId)
        )
      );
    expect(rows).toHaveLength(0);
...
describe('pageOwnedByUser', () => {
  ...
  it('can pin the organization', async () => {
    const rows = await db
      ...
      .where(
        and(
          eq(page.id, pageId),
          pageOwnedByUser(page.id, ownerId, 'some-other-org')
        )
      );
    expect(rows).toHaveLength(0);
  });
  ...
  it("does not match the stranger's page for the owner", async () => {
    const rows = await db
      ...
      .from(page)
      .where(
        and(eq(page.id, strangerPageId), pageOwnedByUser(page.id, ownerId))
      );
    expect(rows).toHaveLength(0);
  });
  ...
  it("still matches the stranger's page for the stranger", async () => {
    const rows = await db
      ...
      .from(page)
      .where(
        and(eq(page.id, strangerPageId), pageOwnedByUser(page.id, strangerId))
      );
    expect(rows).toHaveLength(1);
  });
});
...
describe('blockOwnedByUser', () => {
  ...
  it('does not match for a stranger', async () => {
    const rows = await db
      ...
      .from(block)
      .where(
        and(eq(block.id, blockId), blockOwnedByUser(block.id, strangerId))
      );
    expect(rows).toHaveLength(0);
  });
  ...
  it("does not match the stranger's block for the owner", async () => {
    const rows = await db
      ...
      .from(block)
      .where(
        and(eq(block.id, strangerBlockId), blockOwnedByUser(block.id, ownerId))
      );
    expect(rows).toHaveLength(0);
  });
  ...
  it("still matches the stranger's block for the stranger", async () => {
    const rows = await db
      ...
      .where(
        and(
          eq(block.id, strangerBlockId),
          blockOwnedByUser(block.id, strangerId)
        )
      );
    expect(rows).toHaveLength(1);
  });
...
describe('predicates inside a relational query callback', () => {
  it('finds the page for its owner and not for a stranger', async () => {
    const asOwner = await db.query.page.findFirst({
      where: (p, { and, eq }) =>
        and(eq(p.id, pageId), pageOwnedByUser(p.id, ownerId)),
    });
    expect(asOwner?.id).toBe(pageId);

    const asStranger = await db.query.page.findFirst({
      where: (p, { and, eq }) =>
        and(eq(p.id, pageId), pageOwnedByUser(p.id, strangerId)),
    });
