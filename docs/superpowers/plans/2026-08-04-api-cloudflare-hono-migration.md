...
- **Validated Hono routes: two rules, both learned the hard way in Task 12, both fail silently if broken.**
  1. **Colocate the validator and handler with `hono/factory`'s `createHandlers`, and keep the handler body inline within that call.** Hono's `(path, ...handlers)` overloads infer a handler's `Input` independently of the validator in front of it, so a separately-declared handler typed against the wrong schema — or reading the wrong `.valid()` target — compiles clean and reads `undefined` at runtime. Verified: extracting the handler back out to a named annotated function reopens the hole with zero compile errors. Inline is a convention the type system does not enforce; hold to it.
  2. **Bind Hono's path type parameter to the route's literal string wherever the handler reads a path param.** `c.req.param()` resolves to `string | undefined` otherwise, because neither `createFactory<AppBindings>()` nor a bare `Context<AppBindings>` annotation carries the route path — e.g. `createFactory<AppBindings, '/:blockId/submissions'>()`, `Context<AppBindings, '/:blockId'>`. Discovered in Task 13; Task 12 never hit it because none of its routes had path params.
  3. **Schemas wired into `tbValidator` must be authored with the unscoped `typebox` package, not `@sinclair/typebox`.** `@hono/typebox-validator` peer-depends on `typebox@^1`, and its `Static<T>` does not recognise `@sinclair/typebox` schemas — required fields silently type as optional. Runtime validation is identical either way; only the types diverge. Schemas still serving unported Fastify routes stay on `@sinclair/typebox` until that route is ported, so both packages coexist during the migration.
